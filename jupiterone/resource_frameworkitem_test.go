	...
	return func(s *terraform.State) error {
		...
		for _, r := range s.RootModule().Resources {
			...
			err := resource.RetryContext(ctx, duration, func() *resource.RetryError {
				...
				if err != nil && strings.Contains(err.Error(), "Could not find compliance framework item with id") {
					return resource.RetryableError(fmt.Errorf("FrameworkItem does not exist (id=%q)", id))
				}
	...
	return func(s *terraform.State) error {
		...
		for _, r := range s.RootModule().Resources {
			...
			err := resource.RetryContext(ctx, duration, func() *resource.RetryError {
				...
				if err == nil {
					return resource.RetryableError(fmt.Errorf("FrameworkItem still exists (id=%q)", id))
				}
